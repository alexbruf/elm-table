#!/usr/bin/env sh
# Publish docs-site/dist to Cloudflare Pages as the project elm-table-docs.
#
...
PRESET_API_TOKEN="${CLOUDFLARE_API_TOKEN:-}"
set -a
if [ -f ../.env ]; then
  . ../.env
fi
set +a
# Explicit environment values win over the file.
if [ -n "$PRESET_ACCOUNT_ID" ]; then
  CLOUDFLARE_ACCOUNT_ID="$PRESET_ACCOUNT_ID"
fi
if [ -n "$PRESET_API_TOKEN" ]; then
  CLOUDFLARE_API_TOKEN="$PRESET_API_TOKEN"
fi
# An empty token would make wrangler ignore its own OAuth login.
