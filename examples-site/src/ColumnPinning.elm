module ColumnPinning exposing (main)

{-| Column Pinning.

Ports `examples/react/column-pinning/src/main.tsx` from TanStack Table: the
non-split version, where the pinned columns are simply reordered inside one
table. Every header carries the `<=` / `X` / `=>` pin buttons.

The React example's stress-test button builds 1,000,000 rows. `elm/random`
generates rows one at a time, so this page uses 10,000 instead; everything
else is the same.
...
view model =
    ...
    div [ class "demo-root" ]
        ...
        , div [ class "spacer-md" ] []
        , p [ class "muted" ]
            [ text "This example uses the non-split APIs. Columns are just reordered within 1 table instead of being split into 3 different tables." ]
        , table []
            [ thead [] (List.map (viewHeaderRow model.state) (Table.headerGroups config model.state))
            , tbody [] (List.map (viewRow model.state) (List.take 20 rowModel.rows))
            ]
