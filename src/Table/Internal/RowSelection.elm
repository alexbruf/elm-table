...
canSelectRange cfg state model anchorId row =
    let
        displayRows : List (Row row)
        displayRows =
            Pagination.rowsInDisplayOrder cfg state model
    in
    case ( Dict.get anchorId model.rowsById, displayIndex displayRows anchorId, displayIndex displayRows (Row.id row) ) of
...
displayIndex : List (Row row) -> String -> Maybe Int
displayIndex displayRows rowId =
    displayRows
...
selectRangeWith cfg opts model anchorId row value state =
    let
        ...
        displayRows =
            Pagination.rowsInDisplayOrder cfg state model
    in
    case ( canSelectRange cfg state model anchorId row, displayIndex displayRows anchorId, displayIndex displayRows (Row.id row) ) of
